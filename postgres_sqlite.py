...
def postgres_select_to_sqlite(
    ...
    """
    Execute a read-only SELECT query against Postgres and load the result into a SQLite table.
    ...
    logger.info("Starting Postgres -> SQLite load into table '%s'", target_table)
    ...
    try:
        ...
        logger.info(
            "Completed Postgres -> SQLite load into '%s'. Total rows: %s",
            target_table,
            total_rows,
        )
        return total_rows
        ...
        logger.exception("Failed Postgres -> SQLite load")
        raise RuntimeError(f"Failed to load data from Postgres to SQLite: {exc}") from exc
    finally:
        engine.dispose()
...
rows_loaded = postgres_select_to_sqlite(
    pg_connection_string="postgresql+psycopg://myuser:myp%40ssword@localhost:5432/mydb",
